...
class LatentPretrainingModel(nn.Module):
    """Phase 1 owns backbone, predictors and teachers. It has no decoder member."""
    ...
    def online_parameters(self):
        return list(self.backbone.parameters()) + list(self.image_predictor.parameters()) + list(
            self.imu_predictor.parameters()
        )
...
class RestorationSystem(nn.Module):
    ...
    def decode(self, latent: LatentBatch) -> RestoredBatch:
        image_coeff, imu_coeff = self.decoders(latent.ZI, latent.ZU)
